...
* `converter.apicurio.registry.use-id: true` (or `globalId`): Ensures that only a numeric schema ID is written in the message, matching what Confluent consumers expect.
...
As stated in the Avro link:{link-avro-naming}[documentation], names must adhere to the following rules:
